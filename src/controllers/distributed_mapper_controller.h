...
class DistributedMapperController : public Thread {
 public:
  struct Options {
    ...
    double max_relative_rotation_difference_degrees = 15.0; //20.0; //10.0; //5.0;
    ...
    // my code for just reconstruct a certain cluster
    int test_reconstruct_one_cluster_id = -1;
  ...
  std::vector<Reconstruction> my_reconstructions_;

//  private:
  void Run() override;

  bool SequentialSfM();
  bool SubclusterSequentialSfM();
  bool DistributedSfM();
  ...
  void LoadImages();
  void LoadReplaceabilityMetric(std::unordered_map<ImagePair,float>&replacing_relationships,std::set<image_t>& reserved_imgs); //my code for loading replaceability metric results
  bool LoadTwoviewGeometries();
  bool GenerateMetricGraph(std::unordered_map<ImagePair,float>replacing_relashionships);

  void ClusteringScenesByMetric();
  void GetBestEdges();
  void ClusteringScenes();

  void SubclustersByMetricGraph(std::unordered_map<int, int>& basic_labels, int& subclusters_num);
  void DivideLargeComponents(std::unordered_map<size_t, std::vector<image_t>>& components);
  void MergeSmallComponents(std::unordered_map<size_t, std::vector<image_t>>& components);
  void OutputComponents(std::string filepath, std::unordered_map<size_t, std::vector<image_t>> components);

  void ReconstructSubclusters(std::unordered_map<size_t, ReconstructionManager>& reconstruction_managers,
    std::vector<Reconstruction*>& reconstructions);
  void ReconstructClusters(std::unordered_map<size_t, ReconstructionManager>& reconstruction_managers,
                              std::vector<Reconstruction*>& reconstructions);
  void ReconstructPartitions(std::unordered_map<size_t, ReconstructionManager>& reconstruction_managers,
                             std::vector<Reconstruction*>& reconstructions);
  ...
  void IncrementalSequentialSfM(std::vector<Reconstruction*>& reconstructions,
                     const int num_eff_threads, graph::Node& anchor_node);
  void IncrementalSequentialSfM(std::vector<Reconstruction*>& reconstructions,
                     std::unordered_map<size_t, ReconstructionManager>&
                         reconstruction_managers,
  ...
  bool AdjustGBAForOneMergedRecon(Reconstruction& reconstruction);
  ...
  std::unique_ptr<ImageClustering> metric_clustering_;//my code
  std::unique_ptr<ImageClustering> img_clustering_;//my code
  std::unique_ptr<ImageClustering> subcluster_clustering_;//my code
...
//   std::vector<ReconstructionManager*> my_seq_merged_reconstructions_;
std::vector<ReconstructionManager> my_seq_merged_reconstructions2_;

  std::unordered_set<image_t> separators_;

  VocabSimilarityGraph similarity_graph_;
  ViewGraph view_graph_;
  ViewGraph metric_graph_;
  ...
  std::vector<std::vector<image_t>> subcluster_img_map_;

  std::unordered_map<int, Edges> all_best_edges_;
};
